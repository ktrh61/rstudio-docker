# ==============================================================================
# REBC-THYR PCA Analysis Script v5 - CDM (fast) on logCPM(MUREN) + Pair Consistency
...
source("~/rstudio-docker/utils/utils.R")
source("~/rstudio-docker/utils/norm.R")

# ---- CDM core (as in v3) ----
source("./utils/with_openblas_threads.R")
sourceCpp("./utils/CDM_fast3_arma_enhanced.cpp")

cat("Starting PCA analysis v5: CDM on logCPM(MUREN), protein-coding, MAD top-K...\n")
...
# ==============================================================================
# 3. Helpers: biotype col, logCPM(MUREN) + gene selection, scaling
# ==============================================================================
...
# 置換：05_pca_analysis_v5.R の get_logcpm_for_pca を丸ごとこれに
get_logcpm_for_pca <- function(count_mat, genes_df, samples,
                               K = TOPK_GENES, prior.count = PRIOR_COUNT,
                               var_drop_frac = VAR_DROP_FRAC) {
  # サンプル整備
  smp <- intersect(samples, colnames(count_mat))
  if (length(smp) < MIN_SAMPLES) stop("Too few samples after intersect.")
  stopifnot(nrow(count_mat) == nrow(genes_df))  # 行数の整合
  
  # 対象カウント（ここだけ使う）
  counts_sub <- count_mat[, smp, drop = FALSE]
  
  # ---- MUREN: scaling coefficients（lib.size の対）----
  
  coef <- muren_norm(counts_sub,
                     refs = "saturated",
                     pairwise_method = "lts",
                     refs_cap = Inf,
                     workers = "cl",
  ...
  # edgeR: 有効ライブラリサイズ = coef
  dge <- edgeR::DGEList(counts = counts_sub, genes = genes_df)
  dge$samples$norm.factors <- as.numeric(coef) / dge$samples$lib.size
  logcpm <- edgeR::cpm(dge, log = TRUE, prior.count = prior.count, normalized.lib.sizes = TRUE)
  
  # protein-coding のみに絞る（列名は柔軟に）
  ...
  if (var_drop_frac > 0) {
    thr <- stats::quantile(matrixStats::rowVars(logcpm), probs = var_drop_frac, na.rm = TRUE)
    logcpm <- logcpm[matrixStats::rowVars(logcpm) > thr, , drop = FALSE]
  }
  
  mads  <- matrixStats::rowMads(logcpm)
...
# ==============================================================================
# 5. Run CDM per group on logCPM(MUREN)
# ==============================================================================
cat("Running CDM on logCPM(MUREN) per group...\n")
pca_results <- list()
...
for (analysis_name in names(analysis_groups)) {
  ...
  # logCPM(MUREN) + protein-coding + low-var drop + MAD topK
...
pca_phase1_results_v5 <- list(
  ...
  analysis_version = "v5_cdm_logcpm_MUREN_topK",
...
save(pca_filtered_sample_lists, file = "./data/processed/pca_filtered_sample_lists_v5.rda")
cat("Saved to ./data/processed/ (v5)\n")
