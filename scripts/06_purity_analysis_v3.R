...
# Load the latest PCA results with outlier information
load("./data/processed/pca_phase1_results.rda")
...
# Extract outlier information from PCA Phase 1 results
outlier_detection <- pca_phase1_results$outlier_detection

# Initialize CDM-filtered sample lists
cdm_filtered_sample_lists <- sample_lists

# Apply outlier removal with pair consistency
for (group in c("R0", "R1", "B0", "B1")) {
  cat(sprintf("\n--- Processing CDM outliers for group %s ---\n", group))
  
  if (!group %in% names(sample_lists)) {
    cat(sprintf("Group %s not found in sample lists, skipping...\n", group))
    next
  }
  
  # Get original samples
  orig_tumor <- sample_lists[[group]]$tumor
  orig_normal <- sample_lists[[group]]$normal
  orig_cases <- sample_lists[[group]]$cases
  
  if (length(orig_tumor) == 0) {
    cat(sprintf("Group %s has no samples, skipping...\n", group))
    next
  }
  
  cat(sprintf("Original samples: %d pairs\n", length(orig_tumor)))
  
  # Find outlier samples for this group
  tumor_outliers <- c()
  normal_outliers <- c()
  
  tumor_analysis_name <- paste0(group, "_tumor")
  normal_analysis_name <- paste0(group, "_normal")
  ...
  if (tumor_analysis_name %in% names(outlier_detection) && 
    ...
    cat(sprintf("Tumor outliers found: %d\n", length(tumor_outliers)))
    if (length(tumor_outliers) > 0) {
      cat(sprintf("  %s\n", paste(tumor_outliers, collapse = ", ")))
    }
  }
  ...
  if (normal_analysis_name %in% names(outlier_detection) && 
    ...
    cat(sprintf("Normal outliers found: %d\n", length(normal_outliers)))
    if (length(normal_outliers) > 0) {
      cat(sprintf("  %s\n", paste(normal_outliers, collapse = ", ")))
    }
  }
  
  # Apply pair consistency: remove pairs if either tumor or normal is outlier
  if (length(tumor_outliers) > 0 || length(normal_outliers) > 0) {
    ...
    # Get corresponding case IDs for outliers
    normal_outlier_cases <- if(length(normal_outlier_indices) > 0) orig_cases[normal_outlier_indices] else character(0)
    tumor_outlier_cases <- if(length(tumor_outlier_indices) > 0) orig_cases[tumor_outlier_indices] else character(0)
    ...
    if (length(exclude_cases) > 0) {
      cat(sprintf("Cases with outliers to exclude: %s\n", 
                  paste(exclude_cases, collapse = ", ")))
      ...
      # Update the CDM-filtered sample lists
      cdm_filtered_sample_lists[[group]]$tumor <- final_tumor
      cdm_filtered_sample_lists[[group]]$normal <- final_normal
      cdm_filtered_sample_lists[[group]]$cases <- final_cases
      
      cat(sprintf("Final CDM-filtered samples: %d pairs (removed %d pairs)\n",
                  length(final_cases), length(exclude_cases)))
    } else {
      cat("No outlier pairs to exclude\n")
    }
  } else {
