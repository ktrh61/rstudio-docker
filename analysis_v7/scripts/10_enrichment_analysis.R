...
# Version: v7.8 - Enhanced summary output with primary result details
# Date: 2025-12-04

source("analysis_v7/setup.R")

cat("\n=== Enrichment Analysis for DEG Results (v7.8) ===\n")
cat("Date:", as.character(Sys.Date()), "\n")
cat("Strategy:\n")
cat("  - GSEA for 3 comparisons (excluding B0_vs_B1_normal)\n")
cat("  - Primary result: R0_vs_R1_tumor\n")
cat("  - Cross-comparison 1: RET x BRAF (tumor vs tumor)\n")
cat("  - Cross-comparison 2: Tumor x Normal (within RET)\n")
cat("  - Supplementary: ORA for R0_vs_R1_tumor\n")
cat("\nChanges from v7.7:\n")
cat("  - Enhanced final summary with primary result details\n")
cat("  - Added GSEA results count table\n")
...
# Uses Cliff's delta x min(-log10(q), Q_CAP)
prepare_gsea_genelist <- function(deg_results, gene_mapping, q_cap = CONFIG$Q_CAP) {
  ...
  # Calculate ranking metric: delta x min(-log10(q), q_cap)
...
run_gsea_analysis <- function(comparison_name, thyr_deg_results, gene_mapping, hallmark_t2g, output_dir) {
  ...
  cat(sprintf("    Ranking metric: delta x min(-log10(q), %d)\n", CONFIG$Q_CAP))
...
results_file <- paste0(output_dir, "all_enrichment_results_v7.8.rds")
...
if ("GSEA" %in% names(all_enrichment_results)) {
  ...
  # ----------------------------------------------------------------------------
  # Cross-comparison 1: RET x BRAF (tumor vs tumor)
  # ----------------------------------------------------------------------------
  ...
  # ----------------------------------------------------------------------------
  # Cross-comparison 2: Tumor x Normal (within RET)
  # ----------------------------------------------------------------------------
...
cat("\n=== ENRICHMENT ANALYSIS COMPLETE (v7.8) ===\n")
cat("============================================\n")

# --- Primary Result Summary ---
cat("\n=== PRIMARY RESULT: R0_vs_R1_tumor ===\n")
if ("GSEA" %in% names(all_enrichment_results) && 
    "R0_vs_R1_tumor" %in% names(all_enrichment_results[["GSEA"]])) {
  
  primary_gsea <- all_enrichment_results[["GSEA"]][["R0_vs_R1_tumor"]]
  
  for (db in c("GO_BP", "KEGG", "Reactome", "Hallmark")) {
    result <- primary_gsea[[db]]
    if (!is.null(result) && nrow(result@result) > 0) {
      result_df <- as.data.frame(result)
      n_terms <- nrow(result_df)
      
      # Get top 3 terms (sorted by NES absolute value)
      result_df <- result_df[order(abs(result_df$NES), decreasing = TRUE), ]
      top_terms <- head(result_df, 3)
      
      cat(sprintf("\n  [%s] %d significant pathways\n", db, n_terms))
      for (i in 1:nrow(top_terms)) {
        term_name <- substr(top_terms$Description[i], 1, 45)
        if (nchar(top_terms$Description[i]) > 45) term_name <- paste0(term_name, "...")
        cat(sprintf("    %d. %s (NES=%.2f, q=%.2e)\n", 
                    i, term_name, top_terms$NES[i], top_terms$p.adjust[i]))
      }
    }
  }
}

# --- GSEA Results Count Summary ---
cat("\n=== GSEA RESULTS SUMMARY ===\n")
cat(sprintf("%-20s %8s %8s %8s %8s\n", "Comparison", "GO_BP", "KEGG", "Reactome", "Hallmark"))
cat(paste(rep("-", 60), collapse = ""), "\n")

if ("GSEA" %in% names(all_enrichment_results)) {
  for (comp in CONFIG$GSEA_COMPARISONS) {
    if (comp %in% names(all_enrichment_results[["GSEA"]])) {
      gsea_res <- all_enrichment_results[["GSEA"]][[comp]]
      counts <- sapply(c("GO_BP", "KEGG", "Reactome", "Hallmark"), function(db) {
        if (!is.null(gsea_res[[db]])) nrow(gsea_res[[db]]@result) else 0
      })
      cat(sprintf("%-20s %8d %8d %8d %8d\n", comp, counts[1], counts[2], counts[3], counts[4]))
    }
  }
}

# --- Cross-comparison Summary (from actual results) ---
cat("\n=== CROSS-COMPARISON HIGHLIGHTS ===\n")

if ("cross_comparison" %in% names(all_enrichment_results)) {
  cross_results <- all_enrichment_results[["cross_comparison"]]
  
  # RET Ã— BRAF
  if ("RET_vs_BRAF" %in% names(cross_results)) {
    cat("  RET Ã— BRAF (common across drivers):\n")
    ret_braf <- cross_results[["RET_vs_BRAF"]]
    for (db in c("Hallmark", "Reactome", "KEGG")) {
      if (db %in% names(ret_braf) && !is.null(ret_braf[[db]]$consistent) && 
          nrow(ret_braf[[db]]$consistent) > 0) {
        top_term <- ret_braf[[db]]$consistent[1, ]
        term_name <- substr(top_term$Description, 1, 40)
        cat(sprintf("    [%s] %s (RET=%.2f, BRAF=%.2f)\n",
                    db, term_name, top_term$NES_RET, top_term$NES_BRAF))
      }
    }
  }
  
  # Tumor Ã— Normal
  if ("Tumor_vs_Normal" %in% names(cross_results)) {
    cat("  Tumor Ã— Normal (within RET):\n")
    tumor_normal <- cross_results[["Tumor_vs_Normal"]]
    for (db in c("Hallmark", "GO_BP")) {
      if (db %in% names(tumor_normal) && !is.null(tumor_normal[[db]]$consistent) &&
          nrow(tumor_normal[[db]]$consistent) > 0) {
        top_term <- tumor_normal[[db]]$consistent[1, ]
        term_name <- substr(top_term$Description, 1, 40)
        cat(sprintf("    [%s] %s (Tumor=%.2f, Normal=%.2f)\n",
                    db, term_name, top_term$NES_Tumor, top_term$NES_Normal))
      }
    }
  }
} else {
  cat("  (Cross-comparison results not available)\n")
}
...
cat("\nMethodology:\n")
cat("  GSEA ranking: delta Ã— min(-log10(q),", CONFIG$Q_CAP, ")\n")
cat("  FDR correction: BH method\n")
cat("  FDR threshold:", CONFIG$FDR_CUTOFF, "\n")
