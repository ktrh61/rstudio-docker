...
# ============================================================================
# DEG Analysis Complete - Clean up before visualization
# ============================================================================
...
# (This section can be re-run independently after DEG analysis)
# ============================================================================
...
# Load saved results
cat("Loading saved DEG results...\n")
deg_output <- readRDS(paste0(paths$processed, "thyr_deg_results.rds"))
thyr_deg_results <- deg_output$deg_results
summary_data <- deg_output$summary
comparison_names <- names(thyr_deg_results)
...
# ----------------------------------------------------------------------------
# Calculate unified binwidth and Y-axis for all comparisons
# ----------------------------------------------------------------------------

cat("  Calculating dynamic binwidth and Y-axis...\n")

# Target binwidth: approximately 0.05 for both comparison types
# Cliff's delta discrete step = 2/(n1*n2)
# We find integer multiplier k such that k * step is close to 0.05
...
for (comp_tissue in comparison_names) {
  result <- thyr_deg_results[[comp_tissue]]
  n1 <- result$deg_summary$summary_stats$n_group1
  n2 <- result$deg_summary$summary_stats$n_group2
  
  # Discrete step for Cliff's delta
  step <- 2 / (n1 * n2)
  
  # Find multiplier k to get binwidth close to 0.05
  target_bw <- 0.05
  k <- round(target_bw / step)
  k <- max(k, 1)  # at least 1
  bw <- k * step
  
  binwidth_info[[comp_tissue]] <- list(
    n1 = n1, n2 = n2, step = step, k = k, binwidth = bw
  )
  
  cat(sprintf("    %s: n1=%d, n2=%d, step=%.5f, k=%d, binwidth=%.5f\n",
              comp_tissue, n1, n2, step, k, bw))
  
  # Pre-calculate histogram counts for Y-axis determination
  results_df <- result$deg_summary$results_df
  h <- hist(results_df$cliffs_delta, 
            breaks = seq(-1 - bw/2, 1 + bw/2, by = bw), 
            plot = FALSE)
  hist_counts[[comp_tissue]] <- max(h$counts)
}

# Determine unified Y-axis maximum (rounded up to nice number)
max_count <- max(unlist(hist_counts))
# Round up to next 500
y_max <- ceiling(max_count / 500) * 500
y_breaks <- seq(0, y_max, by = 500)
cat(sprintf("  Unified Y-axis: 0 to %d (max count: %d)\n", y_max, max_count))
...
for (comp_tissue in comparison_names) {
  ...
  # Use dynamically calculated binwidth
  bw <- binwidth_info[[comp_tissue]]$binwidth
...
if (length(effect_size_plots) >= 2) {
  ...
  # y_max and y_breaks already calculated above
  ...
  cat(sprintf("  Saved: effect_size_distributions_combined.pdf (Y-axis: 0-%d)\n", y_max))
}
...
all_deltas <- do.call(rbind, lapply(comparison_names, function(comp_tissue) {
  result <- thyr_deg_results[[comp_tissue]]
...
if ("R0_vs_R1_tumor" %in% comparison_names) {
