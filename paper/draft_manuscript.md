...
【訳】チェルノブイリ事故後の乳頭状甲状腺癌は、ヒト放射線発がんの最も詳細に特徴づけられたモデルであり続けている。その分子的特徴は早期に確立された: 放射線関連腫瘍は高頻度に RET 融合を持つ(Nikiforov et al. 1997; Rabes et al. 2000)。この連関は相関にとどまらない: 照射はヒト甲状腺細胞に RET 再配列を誘導し(Ito et al. 1993)、その生成は線量依存的であり(Caudill et al. 2005)、参加する遺伝子座は甲状腺核内で空間的に隣接している(Nikiforova et al. 2000)。さらに原爆被爆者の乳頭癌でも、高線量群での RET 再配列の優位が独立に観察されている(Hamatani et al. 2008)。REBC-THYR コホートのゲノムランドスケープはこのパターンを大規模に確認した: 線量が上がるほど、腫瘍は点変異でなく融合を driver として生じやすくなる(Morton et al. 2021)。しかし分子的な説明は driver 事象でほぼ止まっている。放射線曝露が — どの driver で始まるかを決めることを越えて — 結果として生じた腫瘍の発現プログラムに検出可能な痕跡を残すのかは、未解決である。
