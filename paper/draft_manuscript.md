...
<!-- N-68 -->。AS 値は解析コードとともに版管理された入力ファイルとして持ち回り、線量・年齢の入力は臨床表と全例照合した。この指標の3つの性質を開示する。第一に、入力経路は IREP の文書が内部被曝核種に処方するものである — 放射線タイプ選択のガイダンスは electrons E > 15 keV の項でヨウ素131を例示として明示的に名指しする — そしてこの区分の放射線有効性係数は参照光子に対して単一値 1.0 である(Land et al. 2003)<!-- N-83 -->。この等価性は、内部ヨウ素131と外部γ線のマイクロドジメトリ解析が独立に支持している(Sato et al. 2014)。第二に、IREP の背後の甲状腺リスクモデルは外部光子被曝 — 原爆被爆者と医療照射を受けた小児(Ron et al. 1995; Kocher et al. 2008)— に当てはめられたものであり、内部ヨウ素131被曝への適用は転送仮定である。曝露率 acute の設定は post-Chernobyl 乳頭癌への先行 IREP 適用(Zurnadzhy et al. 2022; Bogdanova et al. 2022)の規約に従う。chronic を選んだ場合は線量・線量率有効性係数が全線量域に適用され、AS 値は低下する <!-- N-83 -->。第三に、線量は点値として入力されるため、線量測定の不確かさもリスクモデルの不確かさも帯化には伝播しない。これらの性質が AS に求める仕事を限定する: AS は各 driver 層内の事前固定の順序つき帯化指標としてのみ働く — Sporadic 帯は線量ゼロで定義されモデルを要さず、主対比は AS の値でなく帯の所属を比較し、本論文のどの主張も AS の絶対尺度に依存しない。症例は単一の事前固定規則で AS 帯に割り付けた: 非被曝(線量 0)は Sporadic 帯、被曝症例は Low(0 < AS < 33.3)・Mid(33.3 ≤ AS < 66.6)・High(AS ≥ 66.6)。境界上の症例は存在しない <!-- N-69 -->。440 症例全体の driver × 帯 × ペア有無の臨床構成は
表1(仮)に示す <!-- N-11 -->。
...
The contrast-level inferential claim is carried by an omnibus permutation test: the question of whether a contrast carries any label-aligned signal is answered here, not by the size of its gene list. Higher Criticism (α0 = 0.1) is the pre-specified primary omnibus statistic (Donoho & Jin 2004), chosen because it scans the moderately significant range of the ordered p-values rather than resting on a single extreme value or a fixed count threshold, with count- and max-type rows reported descriptively <!-- N-05 -->. The full rejection curve R(α) is retained so that results do not depend on displaying a single threshold count.
...
【訳】対比水準の推論的主張はオムニバス置換検定が担う: 「対比にラベル整列シグナルがあるか」への回答はここで与えられ、遺伝子リストの大きさによってではない。Higher Criticism(α0 = 0.1)が事前指定の主オムニバス統計量である(Donoho & Jin 2004)。選定理由は、単一の極値や固定した件数閾値に依存せず、順序化した p 値の中等度有意域を走査することにある。count 型・max 型の行は記述的に報告する <!-- N-05 -->。棄却曲線 R(α) 全体を保持し、結果が単一閾値の件数表示に依存しないようにする。対比に2値の有意性ラベルは付与しない: 対比水準の証拠は連続量のまま — 遺伝子別 q 値、事前指定オムニバスの p、棄却曲線 — で報告する。これは閾値近傍の証拠の2値化を退ける方法論的勧告(Wasserstein & Lazar 2016; Greenland et al. 2016; Amrhein et al. 2019)に従うものであり、各結果パターンの解釈は結果を見る前に事前割当されている(解釈マップ、Methods)。<!-- C-16 -->
