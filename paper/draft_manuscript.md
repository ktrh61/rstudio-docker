...
これらのゲノム所見は、線量が発現痕跡の軸として誤りであることを示唆する。放射線は起始によって作用する: 線量は、腫瘍が放射線誘発の driver 事象から生じる確率を観察範囲では線形に引き上げる(Morton et al. 2021)。したがって個々の腫瘍は radiogenic か sporadic かのいずれかであり、線量が動かすのは症例系列の構成であって、個々の腫瘍の発現ではない。曝露痕跡があるとすれば、それは症例の身分と共に分離するはずであり — 線量軸に沿っては希釈される。ソースコホート自身の発現の線量回帰が空振りに終わったのは、まさにこの形である。我々は代わりに症例単位の尺度で作業する: 放射線起因性 — Morton et al. (2021) 自身が自然な統合先として名指しした原因確率の量 — を NIH IREP の Assigned Share として計算し(Methods)、各 driver 層内の症例の帯化に用いる。

設計は2つの制約から従う。driver 条件付けは必須であり — 腫瘍発現は driver によって組織化される — それが可能なのは層内に被曝例と非被曝例の両方がある場合に限られる: このコホートでは RET 融合と BRAF V600E である。研究を動機づけたのと同じ線量–融合連関が層の序列も与える: RET の被曝帯は最も高い radiogenic 割合を期待させ、線量関連を持たない BRAF V600E は低事前確率の比較側となる。我々はしたがって、事前指定の4対比 — 各層内の、腫瘍と正常組織での被曝対散発 — を、報告結果が存在する前に固定した解釈マップの下で検定する(Methods、表5(仮)): 一次期待は RET 腫瘍に落ち、正常組織対比が曝露記憶仮説を担い、その層間比較が共有記憶の報告を driver 条件付けの下で検定可能にする。順位ベースの REO パネルが、中間帯で assigned share と共に段階化すると期待される症例別の読み出しを加え、セットレベル機構は実対比に触れる前に較正される(Methods)。本論文はこのプログラムの帰結を報告する。
