...
Contrast-level evidence was summarized with a prespecified Higher Criticism statistic scanning the smallest 10% of the ordered p-values. We chose it to aggregate moderately small p-values rather than depend on one extreme gene or a fixed gene-count threshold. Its p-value came from 9,999 label shuffles within the contrast (seed 19860426) <!-- N-04 -->. Count and maximum statistics and the full rejection curve were retained as descriptive diagnostics. We report these p-values as continuous evidence and do not assign a binary label to each contrast. <!-- C-16 -->
