...
状態: draft(2026-08-14 統合。研究者指示: Methods/Results の対構造の検査可能化と同期機構の廃止のため、旧4ドラフト — draft_methods / draft_results / draft_methods_results_d6 / draft_ora_annotation — を本ファイルへ吸収し削除。内容の変更なし、移設+Discussion 骨組み追加のみ。履歴は git)。規約: 英語本文が正本。Results の【訳】は研究者確認用の対訳(正本でない)。数値は N-ID タグ、主張段落は C-ID タグ(検査対象は英語本文)。図表番号は 2026-08-16 に本置き済み(主: 図1–3・表1–3 / 補足: 図S1–S2・表S1–S8・データ1。BJC の Article 規定 = 主6品目以内に整合)。各節の由来・決定は末尾の「起草メモ(統合)」。
...
データの2つの性質が、この問いを見かけより難しくしている。第一に、腫瘍トランスクリプトームは driver によって組織化される: driver をプールした被曝対散発比較は、driver の頻度自体が線量とともに変わるため(Morton et al. 2021)、曝露と driver 構成を交絡させる。したがって曝露痕跡についてのどの主張も、driver 条件付き比較の内側で立てる必要がある。第二に、曝露の発現痕跡の候補は両組織区画で報告されてきた: 腫瘍・正常組織の線量依存的遺伝子発現 — 組織別には Abend et al. (2013)、腫瘍−正常のペア差としては Abend et al. (2012)、腫瘍組織の放射線マーカーとして提案された CLIP2(Hess et al. 2011)、そして近年、driver 非層別で報告された被曝例の正常組織共有署名(Ory et al. 2026)。しかし driver 条件付けはこの文献群にほぼ欠けている: 線量依存の報告と共有署名の報告は driver の統制を持たず(Abend et al. 2012; 2013; Ory et al. 2026)、7q11/CLIP2 研究は driver 構成を統制した — 群は可能な範囲で RET 再配列の有無でマッチされ、gain は RET 再配列・BRAF 状態のいずれとも連関しなかった(Hess et al. 2011) — が、それはコピー数マーカーについてであり、発現プログラムについてではない。発現水準の曝露痕跡が driver 条件付けに耐えるか否かは、検定されてこなかった。
...
設計は2つの制約から従う。driver 条件付けは必須であり — 腫瘍発現は driver によって組織化される — それが可能なのは、層が被曝例と非被曝例の両方を推論に耐える数で供給する場合に限られる。このコホートでそれを満たすのは RET 融合と BRAF V600E の層だけである。研究を動機づけたのと同じ線量–融合連関が層の序列も与える: RET の被曝帯は最も高い radiogenic 割合を期待させ、点変異 driver であり線量が上がるほど生じにくくなる BRAF V600E は低事前確率の比較側となる。我々はしたがって、事前指定の4対比 — 各層内の、腫瘍と正常組織での被曝対散発 — を、報告結果が存在する前に固定した解釈マップの下で検定する(Methods): 一次期待は RET 腫瘍に落ち、正常組織対比が曝露記憶仮説を担い、その層間比較が共有記憶の報告を driver 条件付けの下で検定可能にする。順位ベースの REO パネルが、中間帯で assigned share と共に段階化すると期待される症例別の読み出しを加え、セットレベル機構は実対比に触れる前に較正される(Methods)。本論文はこのプログラムの帰結を報告する。
...
Gene-level RNA-seq counts (STAR - Counts, open access) for the REBC-THYR project were downloaded from the NCI Genomic Data Commons and md5-verified against the download manifest. The clinical table is Data S1 of the REBC-THYR publication (440 cases <!-- N-11 -->), read without dropping columns or editing values. Expression files were mapped to cases and biospecimen samples through the GDC API and assembled, one count assay per sample, into a single expression matrix (58,448 genes × 906 samples after dropping all-zero genes <!-- N-14 -->). Per-sample strandedness resolution, gene-length derivation and the remaining loading conventions are given in Supplementary Methods.

【訳】REBC-THYR プロジェクトの遺伝子レベル RNA-seq カウント(STAR - Counts、open access)を NCI Genomic Data Commons からダウンロードし、ダウンロードマニフェストに対して md5 検証した。臨床表は REBC-THYR 原論文の Data S1(440 症例 <!-- N-11 -->)で、列の削除も値の編集も行わずに読み込んだ。発現ファイルは GDC API で症例・生体検体に対応づけ、検体あたり単一のカウントアッセイとして1つの発現行列に組み上げた(全ゼロ遺伝子の除去後で 58,448 遺伝子 × 906 検体 <!-- N-14 -->)。検体別の strand 判定・遺伝子長の導出・残りの読み込み規約は Supplementary Methods に示す。

### Exposure metric: assigned share

<!-- 写像元: 130, 140 -->

Radiation attributability per exposed case was quantified with the thyroid model of the National Institutes of Health Interactive RadioEpidemiological Program (NIH IREP, version 5.7.3; Kocher et al. 2008) as the Assigned Share associated with the expected value of the excess relative risk (AS), computed from each case's recorded thyroid dose and ages under a fixed input convention, with the remaining IREP settings at their defaults (the full convention is in Supplementary Methods).

Three properties of this metric are disclosed (in full in Supplementary Methods). First, the input route is the one IREP's documentation prescribes for internal emitters, and it carries a single-valued radiation effectiveness factor of 1.0 relative to reference photons (Land et al. 2003) <!-- N-83 -->. Second, the thyroid risk model behind IREP is fitted to external photon exposure, so its application to internal iodine-131 exposure is a transfer assumption; the acute exposure-rate setting follows the previous IREP applications to post-Chernobyl papillary carcinoma (Zurnadzhy et al. 2022; Bogdanova et al. 2022) and enters the calculation only through the dose and dose-rate effectiveness factor — over the High band (minimum dose 188 mGy <!-- N-84 -->), the chronic alternative would compress the AS scale essentially by one common factor and could not materially reorder cases. Third, the dose enters as a point value, so neither dosimetric nor risk-model uncertainty propagates into the bands. These properties confine what AS is asked to do here: it serves only as a pre-fixed ordinal banding index within each driver stratum — the Sporadic band is defined by dose zero, the primary contrasts compare band membership rather than AS values, and no claim in this paper rests on the absolute scale of AS.
...
<!-- N-69 -->. The clinical composition of all 440 cases across driver × band × pairing is given in Supplementary Table S1 <!-- N-11 -->.

【訳】被曝症例ごとの放射線起因性は、NIH の Interactive RadioEpidemiological Program(NIH IREP、バージョン 5.7.3; Kocher et al. 2008)の甲状腺モデルにより、過剰相対リスク(ERR)の期待値に対応する Assigned Share(AS)として定量した。各症例の記録甲状腺線量と年齢から固定した入力規約で計算し、残りの IREP の設定は既定値のまま用いた(規約の全文は Supplementary Methods)。この指標の3つの性質を開示する(全文は Supplementary Methods)。第一に、入力経路は IREP の文書が内部被曝核種に処方するものであり、参照光子に対する放射線有効性係数は単一値 1.0 である(Land et al. 2003)<!-- N-83 -->。第二に、IREP の背後の甲状腺リスクモデルは外部光子被曝に当てはめられたものであり、内部ヨウ素131被曝への適用は転送仮定である。曝露率 acute の設定は post-Chernobyl 乳頭癌への先行 IREP 適用(Zurnadzhy et al. 2022; Bogdanova et al. 2022)の規約に従い、計算に入る経路は線量・線量率有効性係数のみである — High 帯(最小線量 188 mGy <!-- N-84 -->)の上では、chronic への変更は AS 尺度を実質的に単一の共通倍率で圧縮するのみで、症例の順序を実質的に入れ替えることはできない。第三に、線量は点値として入力されるため、線量測定の不確かさもリスクモデルの不確かさも帯化には伝播しない。これらの性質が AS に求める仕事を限定する: AS は各 driver 層内の事前固定の順序つき帯化指標としてのみ働く — Sporadic 帯は線量ゼロで定義され、主対比は AS の値でなく帯の所属を比較し、本論文のどの主張も AS の絶対尺度に依存しない。症例は単一の事前固定規則で AS 帯に割り付けた: 非被曝(線量 0)は Sporadic 帯、被曝症例は Low(0 < AS < 33.3)・Mid(33.3 ≤ AS < 66.6)・High(AS ≥ 66.6)。境界上の症例は存在しない <!-- N-69 -->。440 症例全体の driver × 帯 × ペア有無の臨床構成は補足表S1に示す <!-- N-11 -->。

### Quality control and analysis cohorts

<!-- 写像元: 210, 220, 230 -->

All eligibility conditions were finalized into explicit per-case flags before any inference, and every downstream script consumes the same flags. Sample outliers were screened before purity estimation — so that an anomalous sample cannot contaminate the purity fit — with PC-scores-based outlier detection (PC-OD; Nakayama et al. 2024; in-house implementation; procedure in Supplementary Methods). Relative tumor purity was then estimated per case with the matched-pair tumor-proportion score of the contamDE family (Shen et al. 2016; Ji et al. 2020) on MUREN-normalized (Feng and Li 2021) paired counts, in one run per driver cohort with both groups (Sporadic and High) together: the pooled run gives both groups one common scale, on which a single threshold has a single meaning (Supplementary Methods), and the estimate serves only as a within-cohort relative filter and a diagnostic covariate. The main analysis cohort comprises driver-classified (RET or BRAF) cases in the Sporadic or High band with a paired tumor/normal sample (pairing rule in Supplementary Methods), both tissues outlier-clean, and pooled relative purity ≥ 0.6 <!-- N-70 --> (n = 63 <!-- N-08 -->); the realized case flow and group composition are reported in Results (Fig. 1; Table 1; Supplementary Table S1). The REO training set is the RET subset of this cohort, and the REO evaluation set is the paired RET tumors of the Low and Mid bands, deliberately left unfiltered by the outlier and purity conditions (its own QC is reported as diagnostics, below).

【訳】全ての適格条件は推論に先立って症例別の明示的フラグとして確定し、下流の全スクリプトが同一のフラグを読む。検体外れ値は純度推定より前に(異常検体が純度の当てはめを汚染しないよう)、PC スコアに基づく外れ値検出(PC-OD; Nakayama et al. 2024; 自作実装; 手続きは Supplementary Methods)でスクリーニングした。相対腫瘍純度はその後、contamDE 系のペア対応腫瘍比率スコア(Shen et al. 2016; Ji et al. 2020)を MUREN 正規化(Feng and Li 2021)したペア付きカウントに適用して症例ごとに推定した — driver コホートごとに両群(Sporadic・High)を合わせた1回の実行である: プールした実行が両群に単一の共通尺度(単一の閾値が単一の意味を持つ尺度)を与え(Supplementary Methods)、推定値の役割はコホート内の相対フィルタと診断用共変量のみである。主解析コホートは、driver 分類済み(RET または BRAF)で Sporadic か High 帯、腫瘍/正常ペアを持ち(ペア解決規則は Supplementary Methods)、両組織とも外れ値クリーン、かつプール相対純度 ≥ 0.6 <!-- N-70 --> の症例からなる(n = 63 <!-- N-08 -->)。実現した症例フローと群構成は Results に報告する(図1; 表1; 補足表S1)。REO 訓練セットはこのコホートの RET 部分集合、REO 評価セットは Low・Mid 帯のペア付き RET 腫瘍で、意図的に外れ値・純度条件で濾過していない(その QC は後述の診断として報告)。
...
<!-- 解釈マップ(計画v2 §0.6 批准マップの転記)。実体 = 「Figure legends and table captions」節の表2キャプション直下(2026-08-18 移設 — 本節は参照文のみ)。C-01/C-04/C-05/C-07/C-16 の読みの参照先 -->
...
【訳】4対比について固定した解釈マップを表2に示す。全ての結果パターンへの読みは、報告結果が存在する前に割り当てられた。
...
<!-- C-15 / C-10(年齢の共変量非投入) -->
Three candidate confounders of the exposure contrasts follow from the design and are disclosed rather than adjusted for; none is tested. The first is age: assigned share is a function of age at exposure, and with a single exposure event the banding itself induces age structure across the bands, so this candidate is guaranteed by construction. The between-group difference in age at surgery (High − Sporadic) is estimated in each driver stratum with 95% bootstrap confidence intervals (estimators in Supplementary Methods) <!-- N-63 -->; this is a disclosure of magnitude and uncertainty, not a confounding test, and no p-value is computed. Age is not entered as a covariate, because age at exposure is a component of the AS definition and adjusting for it would remove part of the exposure metric itself; group age structure is reported descriptively (per-group median and range, Table 1 <!-- N-12, N-13 -->).

The second is sex: sex-chromosome genes differ mechanically between groups of unequal sex composition, so the per-group composition is reported in full (Table 1) and discovered genes are annotated for sex-chromosome membership (sex is not absorbed into the exposure metric; Supplementary Methods). The third is oncogene subtype: the literature associates radiation-related papillary carcinoma with particular RET fusion partners (NCOA4-RET, historically RET/PTC3; Nikiforov et al. 1997; Rabes et al. 2000), so the fusion-partner composition is reported per band (Table 1 <!-- N-12 -->); the BRAF stratum carries a single mutation by construction.

【訳】曝露対比の候補交絡因子3つは設計から導かれ、調整でなく開示する。いずれも検定しない。第一は年齢である: assigned share は被曝時年齢の関数であり、曝露が単一事象であるため、帯化そのものが帯間に年齢構造を誘導する — この候補は構成により保証されている。手術時年齢の群間差(High − Sporadic)を driver 層ごとに 95% ブートストラップ信頼区間つきで推定する(推定量は Supplementary Methods)<!-- N-63 -->。これは大きさと不確かさの開示であって交絡の検定ではなく、p 値は計算しない。年齢は共変量に入れない — 被曝時年齢は AS の定義の構成要素であり、調整は曝露指標そのものの一部除去になるためである。群の年齢構造は記述的に報告する(群別中央値と範囲、表1 <!-- N-12, N-13 -->)。第二は性である: 性構成が異なる群間では性染色体遺伝子が機械的に発現差を生むため、群別の性構成を全数報告し(表1)、発見遺伝子には性染色体所属の注釈を付す(性は曝露指標に吸収されない; Supplementary Methods)。第三は oncogene サブタイプである: 文献は放射線関連乳頭癌を特定の RET 融合パートナー(NCOA4-RET — 歴史的呼称 RET/PTC3; Nikiforov et al. 1997; Rabes et al. 2000)と関連づけるため、融合パートナー構成を帯別に報告する(表1 <!-- N-12 -->)。BRAF 層は構成により単一変異である。
<!-- C-15 -->
...
Each contrast's count matrix (protein-coding genes, filterByExpr, Chen et al. 2016; edgeR) was normalized with a DEGES scheme (Kadota et al. 2012, including its iterated form; the iDEGES designation and the three-iteration default follow Sun et al. 2013) reimplemented in-house: MUREN normalization alternating three times (iDEGES) with an exactly enumerated permutation Brunner–Munzel screen (Storey q < 0.10, Storey 2002) that removes potential DEGs from the scaling-factor estimation, substituted for the original scheme's model-based DEG screen (screen details in Supplementary Methods) <!-- N-71 -->. The per-contrast tested-gene counts, screen pi0 estimates, iteration convergence (Jaccard), and resulting scaling-factor ranges are reported in Supplementary Table S2 <!-- N-15 -->.

【訳】各対比のカウント行列(protein-coding、filterByExpr、Chen et al. 2016; edgeR)は、自作再実装の DEGES 方式(Kadota et al. 2012 — 反復形を含む; iDEGES の呼称と3反復の既定は Sun et al. 2013 に従う)で正規化した: MUREN 正規化と、潜在的 DEG をスケーリング係数推定から除去する完全枚挙の置換 Brunner–Munzel スクリーン(Storey q < 0.10、Storey 2002)との交互反復を3回(iDEGES)— 原方式のモデルベース DEG スクリーンの置換である(スクリーンの細目は Supplementary Methods)<!-- N-71 -->。対比別の検定遺伝子数、スクリーンの pi0 推定値、反復収束(Jaccard)、スケーリング係数の範囲は補足表S2に報告する <!-- N-15 -->。

### Gene-level differential expression

<!-- 写像元: 410(遺伝子別検定・Storey・シャッフルセット定義・pi0 不確かさ) -->

Each gene was tested for an exposed-vs-sporadic difference with the exact permutation Brunner–Munzel test (two-sided; exact enumeration of all C(n, nx) allocations, so the gene-level p-values need no seed). The implementation is an in-house permutation enumeration, run here in its exact mode (its Monte Carlo mode enters only the REO evaluation, below): no asymptotic approximation enters any p-value, and gene-level inference carries neither a count-model assumption nor Monte Carlo error. Gene-level inference is the Storey q-value on the exact p-values with the plug-in pi0 estimate at λ = 0.5, thresholded at the protocol-wide 0.10 level <!-- N-05 -->. The estimator was fixed a priori from the working hypothesis and the design rather than tuned: the hypothesis is a weak signal spread across many genes — small per-gene effects attenuated by within-group case mixture — so holding π0 at 1 would build the absence of exactly that signal into the correction (the full selection argument is in Supplementary Methods). Contrast-level null distributions — for the pi0 uncertainty, the omnibus test and the set-level inference — use one shared set of 9,999 label shuffles drawn at seed 19860426 and saved with the results <!-- N-04, N-05 -->; pi0 is reported with permutation-calibrated uncertainty (the plug-in estimator applied to each saved shuffle, giving its distribution under the contrast's own null).

【訳】各遺伝子は exact 置換 Brunner–Munzel 検定(両側)で被曝対散発の差を検定した — 全 C(n, nx) 割当の完全枚挙のため遺伝子レベルの p 値にシードは不要である。実装は自作の置換枚挙であり、ここでは exact モードで実行する(モンテカルロモードは後述の REO 評価でのみ用いる): どの p 値にも漸近近似は入らず、遺伝子レベルの推論はカウントモデルの仮定もモンテカルロ誤差も持ち込まない。遺伝子レベルの推論は exact p 値に対する Storey q 値(plug-in pi0、λ = 0.5)で、プロトコル共通の 0.10 水準を閾値とする <!-- N-05 -->。推定量はチューニングでなく作業仮説と設計から a priori に固定した: 仮説は多数の遺伝子に広がる弱いシグナル(群内の症例混合で減衰した小さな遺伝子別効果)なので、π0 を 1 に置くことはまさにそのシグナルの不在を補正に組み込むことになる(選定論証の全文は Supplementary Methods)。対比レベルの帰無分布 — pi0 不確かさ・オムニバス検定・セットレベル推論 — は、シード 19860426 で抽出し結果とともに保存した 9,999 回のラベルシャッフルの単一セットを共有する <!-- N-04, N-05 -->。pi0 は置換較正した不確かさ(保存済みの各シャッフルに plug-in 推定量を適用して得る、当該対比自身の帰無の下での分布)を付して報告する。

### Contrast-level omnibus inference

<!-- 写像元: 410(オムニバス・R(α)); 非2値化は C-16 -->

The contrast-level inferential claim is carried by an omnibus permutation test: the question of whether a contrast carries any label-aligned signal is answered here, not by the size of its gene list. Higher Criticism is the pre-specified primary omnibus statistic (Donoho & Jin 2004; the scan range α0 = 0.1 is this protocol's choice), chosen because it scans the moderately significant range of the ordered p-values rather than resting on a single extreme value or a fixed count threshold; its p-value comes from the contrast's own label-permutation null (Supplementary Methods). Count- and max-type rows are reported descriptively <!-- N-05 -->. The full rejection curve R(α) is retained so that results do not depend on displaying a single threshold count.

<!-- C-16 -->
No contrast-level binary significance label is assigned: contrast-level evidence is reported continuously — the per-gene q-values, the pre-specified omnibus p and the rejection curve — following the methodological guidance against dichotomizing evidence near a threshold (Wasserstein & Lazar 2016; Greenland et al. 2016; Amrhein et al. 2019), with the interpretation of each outcome pattern pre-assigned rather than left to post-hoc labeling (interpretation map, Methods).

【訳】対比水準の推論的主張はオムニバス置換検定が担う: 「対比にラベル整列シグナルがあるか」への回答はここで与えられ、遺伝子リストの大きさによってではない。Higher Criticism が事前指定の主オムニバス統計量である(Donoho & Jin 2004; 走査範囲 α0 = 0.1 は本プロトコルの選択)。選定理由は、単一の極値や固定した件数閾値に依存せず、順序化した p 値の中等度有意域を走査することにある。その p 値は当該対比自身のラベル置換帰無から得る(Supplementary Methods)。count 型・max 型の行は記述的に報告する <!-- N-05 -->。棄却曲線 R(α) 全体を保持し、結果が単一閾値の件数表示に依存しないようにする。対比に2値の有意性ラベルは付与しない: 対比水準の証拠は連続量のまま — 遺伝子別 q 値、事前指定オムニバスの p、棄却曲線 — で報告する。これは閾値近傍の証拠の2値化を退ける方法論的勧告(Wasserstein & Lazar 2016; Greenland et al. 2016; Amrhein et al. 2019)に従うものであり、各結果パターンの解釈は事前割当されている(解釈マップ、Methods)。<!-- C-16 -->

### Gene-set level inference

<!-- 写像元: 420, 430(ORA 注釈), lib/gsea_permutation.R, lib/gsea_collections.R -->

Set-level enrichment consumes the whole per-gene ranking (threshold-free; no DEG-list cut decides what is tested), ranked by tie-averaged normal scores of the signed Brunner–Munzel statistic. The enrichment score is an in-house weighted running sum evaluated at tie-block boundaries only, so that no arbitrary tie-break injects an order the data do not contain; on tie-free input it equals the standard GSEA statistic (Subramanian et al. 2005; implementation details in Supplementary Methods) <!-- N-72 -->. The null reuses the identical label shuffles saved by the gene-level test <!-- N-07 -->, so gene- and set-level results rest on one permutation null. Inference is the per-set, sign-conditional permutation p-value with Benjamini–Hochberg adjustment within each collection, q_bh < 0.10 <!-- N-72 -->; no cross-family claim is made. Unlike the gene level, π0 is held at its conservative bound of 1 here: dependent p-values riding one collective drift mode leave the plug-in estimate variance-dominated, with errors pointing anticonservative exactly where the procedure is most fragile. Four MSigDB collections were tested (msigdbr 26.1.0): Hallmark (50; Liberzon et al. 2015), C2:CP restricted to its Reactome, WikiPathways, KEGG MEDICUS, BioCarta and PID subcollections (3,910; the legacy KEGG subcollection is not included), C5:GO:BP (7,538), and a radiation-curated C2:CGP family (28) whose curation rule was fixed before this inference touched real data <!-- N-29, N-55 -->; sets outside the size window 15–500 were excluded <!-- N-72 -->.

<!-- C-06 -->
The complete gene-set inference was calibrated on held-out null replicates — label permutations pushed through the identical procedure — before being applied to the real contrasts, and the choice of the set-level FDR procedure itself was fixed by this calibration prior to any real-data run (Supplementary Methods) <!-- N-06 -->. A spike-in control served as the sensitivity counterpart (configuration in Supplementary Methods). <!-- C-14 -->
As a descriptive complement, the discovered R_Tumor list was annotated by one-sided hypergeometric over-representation against the identical set universe (universe = the contrast's tested genes; details in Supplementary Methods), reported in full as hypothesis-generating annotation (Supplementary Table S3) <!-- N-59 -->. The two procedures' p-values refer to different randomness: only the label-permutation null refers to the experiment actually performed, and experiment-level claims rest on it; the over-representation q-values describe the discovered list against a gene-sampling reference (Goeman & Bühlmann 2007; taxonomy note in Supplementary Methods).

【訳】セットレベルの濃縮は遺伝子別ランキング全体を入力とする(閾値フリー — 検定対象を DEG リストの切断が決めることはない)。ランキングは符号付き Brunner–Munzel 統計量の tie 平均正規スコア。濃縮スコアは tie ブロック境界でのみ評価する自作の重み付き累積和で、データに存在しない順序を恣意的な tie-break で注入しないためであり、tie のない入力では標準 GSEA 統計量(Subramanian et al. 2005; 実装細目は Supplementary Methods)に厳密に一致する <!-- N-72 -->。帰無は遺伝子レベル検定が保存した同一のラベルシャッフルを再利用し <!-- N-07 -->、遺伝子レベルとセットレベルの結果は単一の置換帰無の上に立つ。推論はセット別・符号条件付きの置換 p 値に各コレクション内の Benjamini–Hochberg 調整、q_bh < 0.10 <!-- N-72 --> であり、family 横断の主張はしない。遺伝子レベルと異なりここでは π0 を保守的上限の 1 に置く: 単一の集団ドリフトモードに乗る依存 p 値の下では plug-in 推定は分散支配となり、その誤差は手続きが最も脆いところでちょうど反保守側を向く。MSigDB の4コレクションを検定した(msigdbr 26.1.0): Hallmark(50; Liberzon et al. 2015)、C2:CP のうち Reactome・WikiPathways・KEGG MEDICUS・BioCarta・PID の5下位集合に制限(3,910; レガシー KEGG 下位集合は含めない)、C5:GO:BP(7,538)、放射線キュレーションの C2:CGP ファミリー(28 — キュレーション規則は本推論が実データに触れる前に固定)<!-- N-29, N-55 -->。サイズ窓 15–500 の外のセットは除外した <!-- N-72 -->。セットレベル推論一式は、実対比への適用前に held-out 帰無レプリケート(同一手続きに通したラベル置換)で較正し、セットレベル FDR 手続きの選択自体もこの較正により実データ実行前に固定した(Supplementary Methods)<!-- N-06 -->。spike-in 対照が感度側のカウンターパートである(構成は Supplementary Methods)。<!-- C-06 -->
記述的な補完として、発見済み R_Tumor リストを同一のセット宇宙に対する片側超幾何の過剰代表で注釈した(universe = 当該対比の検定遺伝子; 細目は Supplementary Methods)— 全結果を仮説生成の注釈として報告する(Supplementary Table S3)<!-- N-59 -->。2つの手続きの p 値は異なるランダム性を参照する: 実際に行った実験のランダム性を参照するのはラベル置換帰無のみであり、実験レベルの主張はそこに乗る。過剰代表の q 値は、gene-sampling の参照に対する発見済みリストの記述である(Goeman & Bühlmann 2007; 分類上の注記は Supplementary Methods)。<!-- C-14 -->
...
For each tissue, the exposed-vs-sporadic contrast was summarized per gene by the signed Brunner–Munzel statistic separately in the RET and BRAF strata, and the two genome-wide profiles compared by Spearman correlation over shared genes — threshold-free and without any dose assumption. Each coefficient travels with a permutation-calibrated reference interval — the null spread of rho when neither stratum carries label-aligned structure (shuffle scheme in Supplementary Methods). The normal-tissue pair is the pre-specified hypothesis-bearing comparison; the tumor pair is a descriptive completion. An observed rho outside the interval indicates shared label-aligned structure; by itself it does not identify that structure as an exposure trace rather than a shared covariate.

【訳】各組織で、被曝対散発対比を RET 層・BRAF 層で別々に遺伝子別の符号付き Brunner–Munzel 統計量に要約し、2つのゲノムワイドプロファイルを共有遺伝子上の Spearman 相関で比較した — 閾値フリーで線量仮定なし。各係数には置換較正した参照区間 — どちらの層もラベル整列構造を持たないときの rho の帰無分布 — が付く(シャッフル方式は Supplementary Methods)。正常組織ペアが事前指定の仮説担持比較であり、腫瘍ペアは記述的補完である。区間の外の rho は共有されたラベル整列構造を示すが、それ自体は、その構造が共有共変量ではなく曝露痕跡であることを識別しない。

### REO panel: construction and out-of-sample evaluation

<!-- 写像元: 510, 520, 530 -->

The REO panel provides a per-case, normalization-free readout of the training-band contrast (R_Sporadic versus R_High), built so that it can be examined out of sample in the intermediate bands. Relative expression ordering (REO; the term follows Guan et al. 2016) works on within-sample gene ranks and is therefore free of between-sample normalization — the pairwise-rank primitive is due to Geman et al. (2004) and the reversal-based scheme to RankComp (Wang et al. 2015); expression was taken as TPM, recomputed from the selected count assay and the exon-union gene lengths because within-sample comparison across genes requires length normalization (Supplementary Methods). Candidate pairs were generated in the RET tumor training groups from the top 500 genes by Brunner–Munzel effect magnitude (a threshold-free pool; the q < 0.10 DEG set is deliberately not used), qualified and ranked by pre-fixed stability and reversal rules, and selected greedily to a target of 10 pairs (rules in Supplementary Methods) <!-- N-37 -->. A sample's reversal score is the number of panel pairs whose within-sample difference clears the dead zone (Supplementary Methods) and runs opposite to the training-Sporadic reference sign (an integer 0–10); the classification boundary is training-derived (positive above the maximum reversal score among the training Sporadic samples), and the realized panel, boundary and construction yield are reported in Results and Supplementary Table S4 <!-- N-39 -->.
...
【訳】REO パネルは、訓練帯対比(R_Sporadic 対 R_High)の症例別・正規化フリーの読み出しを与える計器であり、中間帯で out-of-sample 検査ができるように構築する。相対発現順序(REO; 用語は Guan et al. 2016 に従う)は検体内の遺伝子順位で動作し、検体間正規化から自由である — ペア順位の原始は Geman et al. (2004)、逆転に基づく方式は RankComp(Wang et al. 2015)による。発現は TPM とした — 検体内での遺伝子間比較が長さ正規化を要求するため、採用カウントアッセイと exon-union 遺伝子長から再計算した(Supplementary Methods)。候補ペアは RET 腫瘍訓練群で、Brunner–Munzel 効果の大きさ上位 500 遺伝子から生成し(閾値フリーのプール; q < 0.10 の DEG 集合は意図的に使わない)、事前固定の安定性・逆転規則で適格化・順位付けし、貪欲選定で目標 10 ペアとした(規則は Supplementary Methods)<!-- N-37 -->。検体の逆転スコアは、パネルのうち検体内差が dead zone(Supplementary Methods)を越えかつ訓練 Sporadic の参照符号と逆向きのペア数である(0–10 の整数)。分類境界は訓練由来である(訓練 Sporadic 検体の最大逆転スコアを越えたら positive)。実現したパネル・境界・構築の歩留まりは Results と補足表S4に報告する <!-- N-39 -->。確定したパネルは、訓練に使っていない中間帯 RET 腫瘍(R_Low・R_Mid <!-- N-10 -->)へ変更なしで適用し、段階的な out-of-sample 検査とした。訓練2帯はパネル構築に入っているため帯内の分離は循環であり、スコアを out-of-sample で検査できる帯は Low と Mid のみで、設計が方向を事前に指定する(assigned share の高い帯が上)。したがって唯一の事前指定検定は Mid over Low の片側 Brunner–Munzel 比較である(モンテカルロ、シード 19860426 <!-- N-75 -->)。この評価はパネルにも境界にも変更を加えず、帯別スコアプロファイルは線量反応の形を仮定せず記述的に報告する。<!-- C-08 の手法側 -->
...
<!-- C-09 -->
Because the evaluation cohort is deliberately unfiltered, its QC is reported as diagnostics with no exclusion authority: the training outlier screen mirrored on the evaluation tumors (counts reported only), tumor purity on the training-common scale, and <!-- C-12 --> — because the reversal score correlates with purity within the evaluation bands, so a graded band profile could in principle be purity-driven rather than band-driven — a separation of band from purity by partial correlation and purity-stratified comparisons (procedures in Supplementary Methods).
...
The publication run executed R 4.5.3 on Ubuntu 24.04 <!-- N-02 --> with the reference BLAS/LAPACK 3.12.0 <!-- N-03 -->, four workers and 9,999 label shuffles as the fixed reproduction contract <!-- N-04 -->, from a clean repository state <!-- N-01 -->, in an environment rebuilt from a fully date-pinned container build (Supplementary Methods). The canonical inference seed is 19860426; diagnostics draw documented seeds from base 19450809 <!-- N-05, N-06 -->. The full pipeline was executed independently on two x86-64 machines, with md5-identical raw inputs (1,819 files <!-- N-52 -->), the test suite passing on both (415 tests, 0 failures <!-- N-51 -->) and primary artifacts verified identical; the reproduction contract, including this bit-level identity, is defined on x86-64 Linux and is not asserted for other architectures. The core statistical machinery is implemented in-house in the analysis code rather than taken from packages (components enumerated in Supplementary Methods), with method sources cited at first mention; external packages actually used are pinned in the container build and listed with versions in Supplementary Table S5. Analysis code, versioned inputs and the container build recipe sufficient to regenerate the reported analyses accompany the paper.
...
【訳】本番実行は R 4.5.3 / Ubuntu 24.04 <!-- N-02 -->、参照 BLAS/LAPACK 3.12.0 <!-- N-03 -->、ワーカー 4・ラベルシャッフル 9,999 を固定された再現契約とし <!-- N-04 -->、クリーンなリポジトリ状態から <!-- N-01 -->、全層を日付固定したコンテナビルドで再構築される環境(Supplementary Methods)で実行した。正準の推論シードは 19860426、診断は基底 19450809 から文書化されたシードを引く <!-- N-05, N-06 -->。全パイプラインは2台の x86-64 マシンで独立に実行され、生入力は md5 一致(1,819 ファイル <!-- N-52 -->)、テストスイートは両機で通過(415 テスト、失敗 0 <!-- N-51 -->)、主要成果物は同一と検証された。この再現契約はビットレベル同一性を含めて x86-64 Linux 上で定義され、他のアーキテクチャについては主張しない。核心の統計機構はパッケージからでなく解析コード内の自作実装であり(構成要素の列挙は Supplementary Methods)、手法の原典は初出箇所で引用する。実際に使用した外部パッケージはコンテナビルドで版固定され、補足表S5に版つきで一覧する。報告した解析の再生成に足る解析コード・版管理された入力・コンテナビルドレシピを論文に添付する。パイプラインの設計・原型スクリプト・全ての解析上および解釈上の判断は著者らによる。生成 AI アシスタント(Claude Fable 5, Anthropic; GPT-5.6sol, OpenAI)は、スクリプトのデバッグ・リファクタリングに加え、著者らの細粒度の指揮の下で原稿本文の起草・編集に用いた。文章支援が向けられたのは統計的正確性である: 推定対象・推論のスコープ・各主張が伴う限定句を正確に文言化し、技術用語の用法を原稿全体で一貫させること。起草された文章は著者らが指定し批准した内容を文章化したものであり、版管理された台帳群 — 全数値は解析出力と、全引用は原典と、主張担持文の文言は著者批准の主張マップと — に対して検証し、各段階で著者の承認を経た。科学的判断はこれらのツールに委譲していない。ツールが扱ったのは open-access データのみであり、結果としての全コードは上述の検証(テストスイート・機械間再現・自動等価性検査)を通過している。内容の全体について著者らが全責任を負う。
...
**Table 2 | Interpretation map for the four exposure contrasts, fixed before the reported results existed.** Each contrast carries a pre-specified standing and its basis; the pattern rules fixed together with the map are given as table footnotes. No outcome pattern requires post-hoc labelling.
