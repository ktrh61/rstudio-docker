...
The genomic findings themselves motivate a framing different from dose–response. Radiation acts by initiating — dose increases, linearly in the observed range, the probability that a tumor arises from a radiation-induced driver event (Morton et al. 2021) — so an individual tumor either is radiogenic or is sporadic, and dose shifts the composition of a case series rather than the expression of any single tumor. On this view, a linear-in-dose regression of expression is the wrong instrument: any exposure trace would segregate with case status and be diluted along a dose axis, consistent with the absence of dose-associated expression patterns in the source cohort's own analyses (Morton et al. 2021). We therefore organize the design around per-case attributability — the probability-of-causation scale that Morton et al. (2021) identified as the quantity into which radiation-associated characteristics should be incorporated — quantified as the NIH IREP Assigned Share (Methods), and band cases on it, so that each driver stratum contains an unexposed Sporadic group and bands of increasing expected radiogenic fraction, with the extreme bands providing the cleanest contrast and the intermediate bands a graded out-of-sample test bed.

Driver conditioning constrains where such a comparison can be made at all: it requires exposed and unexposed cases within the same stratum, and RET fusions and BRAF V600E are the two drivers for which the cohort supplies both in analyzable numbers (Methods). The same evidence that motivates the design also ranks the strata: fusion drivers — most commonly RET — are the dose-enriched events (Nikiforov et al. 1997; Rabes et al. 2000; Morton et al. 2021), so under the mixture view the exposed bands of the RET stratum carry the highest expected radiogenic fraction, whereas BRAF V600E shows no dose association and its stratum serves as the low-prior comparison. We therefore built a pre-specified analysis around four driver-conditioned contrasts — exposed versus sporadic in tumor and in normal tissue, within each stratum — with three declared inference levels (per-gene q-values, a per-contrast omnibus test carrying the contrast-level claim, and a calibrated gene-set level), and we fixed an interpretation map before the reported results existed, so that every outcome pattern carries a pre-assigned reading (Methods). The map places the primary expectation on RET tumors for exactly this reason and treats the normal-tissue contrasts as the carriers of the exposure-memory hypothesis; the cross-stratum comparison of the two normal contrasts is the device that makes the shared-memory reports testable under driver conditioning. Two further instruments extend the design: a rank-based REO panel built on the training bands provides a per-case, normalization-free readout, with the pre-specified expectation that a panel capturing a radiation-attributable signal should grade with assigned share across the intermediate bands; and the complete set-level machinery is calibrated on held-out null replicates and a synthetic positive control before touching the real contrasts. This paper reports the outcome of that program.
...
ゲノム所見そのものが、線量反応とは異なる枠組みを動機づける。放射線は起始によって作用する — 線量は、腫瘍が放射線誘発の driver 事象から生じる確率を、観察範囲では線形に引き上げる(Morton et al. 2021)。したがって個々の腫瘍は radiogenic か sporadic かのいずれかであり、線量が動かすのは症例系列の構成であって、個々の腫瘍の発現ではない。この見方の下では、発現の線量線形回帰は誤った計器である: 曝露痕跡があるとすれば、それは症例の身分と共に分離し、線量軸に沿っては希釈される — ソースコホート自身の解析で線量関連の発現パターンが不在だったこと(Morton et al. 2021)と整合する。我々はしたがって設計を症例別の起因性 — Morton et al. (2021) が放射線関連特性の統合先として名指しした原因確率の尺度 — の周りに組織し、それを NIH IREP の Assigned Share として定量して(Methods)症例を帯化する。これにより各 driver 層は、非被曝の Sporadic 群と、期待される radiogenic 割合が増す帯を持ち、両端の帯が最も明瞭な対比を、中間帯が段階的な out-of-sample の試験台を与える。

driver 条件付けは、そもそもこの比較が可能な場所を制約する: 同一層内に被曝例と非被曝例の両方が必要であり、コホートがその両方を解析可能な数で供給する driver は RET 融合と BRAF V600E の2つである(Methods)。設計を動機づけたのと同じエビデンスが層の序列も与える: 融合 driver — その最頻が RET — こそが線量とともに濃縮される事象であり(Nikiforov et al. 1997; Rabes et al. 2000; Morton et al. 2021)、混合観の下では RET 層の被曝帯が最も高い radiogenic 割合を期待させる。一方 BRAF V600E は線量関連を示さず、その層は低事前確率の比較側となる。我々はしたがって、4つの driver 条件付き対比 — 各層内の、腫瘍と正常組織での被曝対散発 — の周りに事前指定の解析を構築し、3つの宣言済み推論水準(遺伝子別 q 値・対比水準の主張を担う対比別オムニバス・較正済みセットレベル)を置き、報告結果が存在する前に解釈マップを固定して、どの結果パターンにも事前割当の読みが付くようにした(Methods)。マップはまさにこの理由で一次的期待を RET 腫瘍に置き、正常組織対比を曝露記憶仮説の担い手として扱う。二つの正常対比の層間比較は、共有記憶の報告を driver 条件付けの下で検定可能にする装置である。さらに2つの計器が設計を拡張する: 訓練帯上に構築した順位ベースの REO パネルは症例別・正規化フリーの読み出しを与え、放射線起因性シグナルを捉えるパネルならばスコアは中間帯で assigned share と共に段階化するはずだという事前期待を持つ。セットレベル機構一式は、実対比に触れる前に held-out 帰無レプリケートと合成陽性対照で較正される。本論文はこのプログラムの帰結を報告する。
