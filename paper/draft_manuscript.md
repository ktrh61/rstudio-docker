...
Two features of the data make this question harder than it appears. First, tumor transcriptomes are organized by their driver: an exposed-versus-sporadic comparison that pools drivers confounds exposure with driver composition, because driver prevalence itself varies with dose (Morton et al. 2021). Any claim about an exposure trace must therefore be made within driver-conditioned comparisons. Second, the reported traces of exposure in thyroid tissue concern the normal gland: long-term expression memory in normal thyroid (Abend et al. 2013) and, recently, shared normal-tissue signatures in exposed cases reported without driver stratification (Ory et al. 2026). Whether those normal-tissue observations survive driver conditioning, and whether any counterpart exists in the tumors themselves, has not been tested.

[Hypothesis statement — 研究者記入スロット: バイナリ観(症例は radiogenic / sporadic の二値であり、帯差は混合比の反映)の定式化がここに入る。前後の文はこのスロットを前提に接続してある。] Radiation attributability is quantified per exposed case as the NIH IREP Assigned Share (Methods) and cases are banded on it, so that each driver stratum contains an unexposed Sporadic group and exposure bands of increasing attributability.

We therefore built a pre-specified analysis around four driver-conditioned contrasts — exposed versus sporadic in tumor and in normal tissue, within the RET and BRAF strata — with three declared inference levels (per-gene q-values, a per-contrast omnibus test carrying the contrast-level claim, and a calibrated gene-set level), and we fixed an interpretation map before the reported results existed, so that every outcome pattern carries a pre-assigned reading (Methods). The map places the primary expectation on RET tumors and treats the normal-tissue contrasts as the carriers of the exposure-memory hypothesis; the cross-stratum comparison of the two normal contrasts is the device that makes the shared-memory reports testable under driver conditioning. Two further instruments extend the design: a rank-based REO panel built on the training bands provides a per-case, normalization-free readout, with the pre-specified expectation that a panel capturing a radiation-attributable signal should grade with assigned share across the intermediate bands; and the complete set-level machinery is calibrated on held-out null replicates and a synthetic positive control before touching the real contrasts. This paper reports the outcome of that program.
...
データの2つの性質が、この問いを見かけより難しくしている。第一に、腫瘍トランスクリプトームは driver によって組織化される: driver をプールした被曝対散発比較は、driver の頻度自体が線量とともに変わるため(Morton et al. 2021)、曝露と driver 構成を交絡させる。したがって曝露痕跡についてのどの主張も、driver 条件付き比較の内側で立てる必要がある。第二に、甲状腺組織で報告されている曝露の痕跡は正常腺に関するものである: 正常甲状腺の長期発現記憶(Abend et al. 2013)、そして近年、driver 非層別で報告された被曝例の正常組織共有署名(Ory et al. 2026)。これらの正常組織の観察が driver 条件付けに耐えるのか、腫瘍自体に対応物があるのかは、検定されてこなかった。

[仮説文 — 研究者記入スロット] 放射線起因性は被曝症例ごとに NIH IREP の Assigned Share として定量され(Methods)、症例はその帯で層化される — 各 driver 層は非被曝の Sporadic 群と、起因性が増す被曝帯を持つ。

我々はしたがって、4つの driver 条件付き対比 — RET・BRAF 各層内の、腫瘍と正常組織での被曝対散発 — の周りに事前指定の解析を構築し、3つの宣言済み推論水準(遺伝子別 q 値・対比水準の主張を担う対比別オムニバス・較正済みセットレベル)を置き、報告結果が存在する前に解釈マップを固定して、どの結果パターンにも事前割当の読みが付くようにした(Methods)。マップは一次的期待を RET 腫瘍に置き、正常組織対比を曝露記憶仮説の担い手として扱う。二つの正常対比の層間比較は、共有記憶の報告を driver 条件付けの下で検定可能にする装置である。さらに2つの計器が設計を拡張する: 訓練帯上に構築した順位ベースの REO パネルは症例別・正規化フリーの読み出しを与え、放射線起因性シグナルを捉えるパネルならばスコアは中間帯で assigned share と共に段階化するはずだという事前期待を持つ。セットレベル機構一式は、実対比に触れる前に held-out 帰無レプリケートと合成陽性対照で較正される。本論文はこのプログラムの帰結を報告する。
...
| R_Normal | Hypothesis-bearing; signal possible | long-term exposure memory reported in normal thyroid (Abend et al. 2013; Ory et al. 2026) |
