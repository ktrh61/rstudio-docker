...
これは記述的な包含関係であって濃縮検定ではなく、driver非層別群から得られたOryの特徴選択解析を再現するものでも、署名を検証するものでもありません。また、RET-tumorのq<0.10リストが検定対象遺伝子の約11.3%を含むことを考えると、観察された重なり自体を独立した肯定的証拠として扱うこともできません。Discussionでは、主結果を補強する材料ではなく、先行研究との位置関係を正確に示すための記述的照合として扱います。
