...
追加計算は、第二の有意差検定ではなく、既存の R_Tumor 結果に大きさと共変量の文脈を与える記述的監査に限定して実行しました。QC、正規化、遺伝子検定、多重性調整は再実行せず、新しい p 値や別の遺伝子リストも作成していません。
...
これにより、p/q 値とは別に差の大きさを提示できますが、第二の「陽性/陰性」判定は生じません。
...
5,000語版の Methods、Results、Discussion と補足図案へ挿入しました。PCA は共変量が「問題なし」と証明する図ではなく、単一共変量による単純な分離の有無と、複合構造を読者が確認するための記述図です。
