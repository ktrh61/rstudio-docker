...
The acute setting was retained to preserve computational continuity with earlier IREP applications to post-Chernobyl papillary carcinoma from the same research programme (Zurnadzhy et al. 2022; Bogdanova et al. 2022). These studies document the provenance of the convention rather than independently validating acute coding for protracted internal iodine-131 exposure. In IREP, exposure rate affects the calculation through the dose and dose-rate effectiveness factor. Under the acute setting it approaches 1 at and above an uncertain reference dose distributed log-uniformly from 0.03 to 0.2 Gy; the High band's minimum dose was 188 mGy. A chronic calculation lowers the numeric AS scale. In an input-convention audit based only on the paired AS calculations and not on expression outcomes, chronic coding preserved the ordering of exposed cases, with no pairwise rank inversions. This audit establishes stability of the ordering, not the physical validity of acute coding or the invariance of fixed numerical cut-points across calculation scales.
