...
| Table S2 | `table_s2_ora_annotation.csv` | `tables/supp_tab_ora_annotation.R`(processed/thyr_deg_ora_annotation.rds の $table、18,576 行) | 2026-08-21 作表・スクリプト化。q<0.10 員数は N-59 と全一致、スクリプト出力と本ファイルは diff 同一 |
| Table S3 | `table_s3_reo_panel.csv` | Frozen `output/reo_panel.csv` from `scripts/510_select_reo_pairs.R` | Available |
| Table S4 | `table_s4_software_versions.csv` | `tables/supp_tab_package_versions.R` | Available |
| Table S5 | `table_s5_gene_set_summary.csv` | `tables/tab_set_level_summary.R` | Available |
| Table S6 | `table_s6_complete_null_calibration.csv` | `tables/tab_set_level_summary.R` | Available |
| Table S7 | `table_s7_between_stratum_concordance.csv` | `tables/supp_tab_concordance.R` | Available |
