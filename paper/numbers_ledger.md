...
- 語数: **投稿版基準([n] 変換後 — ジャーナルが見る形)が判断の正**(研究者決定 2026-08-31)。正本基準
  (author-year のまま・見出し除く)は参考。最新値は語数記載のある直近の改訂メモ参照(本文 = 見出し込み Word 4,904、2026-09-02)。Abstract 200
  (計数規則 = 小見出し 4 語込み・"Abstract" 表題除く、Word 計数 2026-09-02、余裕 0)。留意: 引用の新規追加は投稿版でも [n] 分増える。機械検査(凍結4・N/C タグ・引用 33/
  警告0・英綴り・訳の段落/数値同期・SI 初出順・**略語 4 回未満は綴り出し(BJC house style)**・引用 [n] 角括弧)全通過 2026-09-02。
...
- 開いている残件(文言の未決論点はゼロ — 残りは批准・記入・フェーズ2 のみ):
  ...
  6. 公開リポジトリ(published-repo-scope: 論文再現に必要なスクリプト・lib・figures・tables・Dockerfile・
     ...
     環境同一性確認(生入力は 2026-08 期間中不変のため公開実行の再現主張は現行のまま有効)
- **日本語参考訳の同期規則(2026-08-27 批准、2026-08-29 カバーレターへ拡張)**: paper/manuscript_ja.md・
  paper/supplementary_ja.md・paper/cover_letter_ja.md は関係者共有用の参考訳(正本は英語・共著者は
  英語版のみ)。本文・補足資料・カバーレターを修正するコミットは、対応する訳の該当段落の同期修正を
  同一コミットに含める(訳出規律 = 限定句完全保存・冒頭の用語固定表)。
...
- 2026-08-26: 閲覧用 Supp docx に小表7枚(S1・S2・S4〜S8、出荷用コピーの一方向埋め込み)を
  追加(研究者 Go)。S3(18,577行)・Data 1/2 は物理的に埋め込み不能のためキャプションのみ —
  投稿時の別ファイル構造と整合。S1 の非解析例は no_reference カテゴリで AS 帯なし
  (B.11 の IREP 置換で近似は退役済み — 研究者懸念を実測で解消)
- 2026-08-27: **Supplementary References 節を生成時付加**(親族指摘由来 — Supp の author-year
  引用 37 件が投稿物内で書誌解決できない欠落を解消。台帳を単一情報源に自己完結リストを
  自動生成、不一致は警告検査に乗る)。付随して load_ledger の 2 バグ修正: 年抽出が表題中の
  年(Land 1985)を拾う→構造優先へ/同姓同年キー衝突(Morgan 2025 ×2)→先勝ち+第2著者
  キーのソフトウェア型対応。本文 References 31 件は回帰ディフで同一を確認・警告 0
- 2026-08-27: **Neubert & Brunner 2007 を追加**(研究者指摘 — studentized 置換 BM 検定の
...
- 2026-08-27: **日本語参考訳を新設**(paper/manuscript_ja.md、追跡ファイル — フェーズ1
  「対訳再生成」の宛先裁定 = 共著者は英語版のみ・日本語訳は関係者共有用、対訳並記は不出荷)。
  対象版 e163e8e 系列・段落 1:1・限定句保存・用語固定表つき。同期規則を現況節に批准記録
- 2026-08-27: 補足資料の日本語参考訳を新設(paper/supplementary_ja.md — 対象版 e163e8e 系列・
  段落 1:1・限定句保存・用語固定表は本文訳と共通+追加分)。同期規則を両訳へ拡張
- 2026-08-27: 両訳へ可読性パス(研究者 Go — 限定句・否定・数量語トークンを不変条件に構文のみ
  自然化、機械差分で主張ずれゼロ確認)。付随して訳の忠実性2件是正: dosimetric=線量計測(脱落
  +空白混入)・separate proof=別個(独立と誤訳)
- 2026-08-27: make_docx へ日本語訳ビルドを統合(研究者 Go — 同一時刻タグで英日4ファイル同時
  生成・訳 docx 冒頭に対応英語版名+ソースhashを自動記載・和文書体 游明朝10.5pt+TNR/游ゴシック
  見出し/行番号なし)。遺伝子イタリック対象に PXDN・P2RY1・PLK2 を補完(英語版 Supp も同欠落だった)
- 2026-08-27: 訳 docx の組版是正(研究者指摘 — 読みにくい): 両端揃え撤回(英字トークンで字間
  が間延び)・行送りを固定 18pt atLeast へ(游書体の縦メトリクス過大で 1.5 倍指定が倍行送りに
  見える問題)・冒頭メタ箇条書きを 9pt 灰色に圧縮
- 2026-08-27: 訳 docx を BIZ UD 仕様へ(研究者裁定 — 共有は PDF 前提で埋め込みにより他環境の
  置換懸念消滅、画面可読性優先): 本文 BIZ UD明朝 Medium 11pt・見出し BIZ UDゴシック 14/12.5/11pt・
  行送り 19pt。対応版行を「。」区切りへ、両訳ヘッダ用語固定表の / 区切りに半角スペース(折返し点の確保)
- 2026-08-27: Supp Normalization に MUREN の LTS エンジンが原実装と同一である旨を明記(研究者
  指摘 → 上流照合で「相当」でなく「同一」を確認: NAMESPACE importFrom(MASS,ltsreg)・同式)。
  訳同期済み。docx 再生成は次回まとめ(研究者指示)
- 2026-08-27: オムニバス節で主要統計量の構成を自己完結化(研究者 Go): 内部 p = プールシャッフル
  帰無への経験的 p・HC+ 形(p≤1/n 除外)・plus-one 計数(HC/個数/最大値の 3 行)を明記 —
  410 のコードと一致確認済み。訳同期済み・docx はまとめ待ち
- 2026-08-27: Supp AI 節に指揮層の 1 文を追加(研究者 Go — 出力受領・検証層のみで著者主導の
  ニュアンスが弱い): author-directed・自作パイプライン先行(initially implemented)・自由目標の
  不付与を肯定形で明記。訳同期済み・docx はまとめ待ち
- 2026-08-27: Supp REO 結果の検証否定文を是正(研究者 Go — 通読違和感): 解釈枠に時制統一
  (did→does)+external を除去し方法節の否定幅(independent validation)と一致。全体走査で
  同型(無動機の時制混在・否定射程の不揃い)は他に無しを確認。訳同期済み・docx はまとめ待ち
...
- 2026-08-29: **表 S1 を解析枠準拠へ再改稿**(研究者決定 — 「表に載る AS は解析に使った AS のみ」):
  行を指定ドライバー目録(Morton 転載に近い)から解析層へ切り替え(RET 層パートナー別+小計・
  BRAF V600E 層・層外 3 行・全 440)、層外の AS(入力 CSV の未使用 14 例分)は不表示。層小計
...
- 2026-08-29: **表 S1 を廃止し Table 2 へ統合**(研究者決定): 各解析群の層×帯母数(ペアあり)を
  ...
  対応正常を持つのは 31)
- 2026-08-29: Limitations に標本サイズの限定 2 文を追加(研究者 Go): 群 9–27 例(N-09/N-10)、正確・
  並べ替え手続きで妥当性は漸近近似に非依存だが検出力は限定、副次の帰無は不在の証拠でない、
...
- 2026-08-29: RET-tumor リストの X/Y 連鎖数(N-82)を本文 Results へ復帰(研究者 Go、+約 28 語):
  57/1,765 対 背景 572/15,621、Supp の限定句(記述的・性関連構造を除外しない)を同伴。訳同期済み
- 2026-08-29: **SI 品目を初出順へ再番号付け**(通読前検査で発見・研究者 Go — 旧番号でも初出順でなかった):
  表(旧→新)= 正規化診断 S1→S1・版 S4→S2・セット要約 S5→S3・較正 S6→S4・ORA S2→S5・層間一致
...
- 2026-08-29: **docx を「Word → PDF 書き出しで提出品質」へ**(研究者指示): 埋め込み図を 300 dpi PNG に
  (160 dpi から。投稿用の別ファイルは従来どおり PDF ベクター+600 dpi TIFF)、表セルの本文を 10 pt 黒で
  run に明示(和文版のメタ箇条書き 9 pt 灰色が表 1 に波及していた副作用も解消)、Supp の補足表区間を
  横置きセクションに(S1 12 列・S4 10 列)、各セクションにフッタ頁番号と行番号を明示。SVG 埋め込みは
  Word の描画差と 6 万点散布図の重さから不採用。**PDF 書き出し手順**(Word): オプション → 詳細設定 →
  「ファイル内のイメージを圧縮しない」ON → 名前を付けて保存 → PDF、最適化 = 標準、PDF/A は不使用
  (フォント埋め込みは既定)。完成水準の判定は研究者の確認を経て(2026-08-29 時点は理論整合まで)
- 2026-08-29: 表の列幅を autofit へ(固定均等幅で見出し語が分断されていた)+上下罫線。
  「イメージを圧縮しない」は文書単位設定のため docx 生成時に settings.xml へ
  doNotAutoCompressPictures を埋め込み(開くたびの再設定を不要化)。手順から当該項目を削除
- 2026-08-29: 補足表の横置きを表単位へ(S1・S4 のみ。区間まるごと横置きでは S2 が分割され右側が
  空いた — 研究者指摘)。表内は 1 倍行送り(縦長の S2 を縦置き 1 頁に)。区間構成 P|L|P|L|P、
  全区間にフッタ頁番号
- 2026-08-29: **表 S2 を「実際に読み込むパッケージ」へ限定**(研究者指摘 — TCC は lib/norm_deges.R の
  コメントで手法の準拠先として言及されるだけで呼ばれていない): 生成スクリプトがコードを走査して
  library/requireNamespace/:: を収集し(コメント行は除外)、versions.tsv の固定版と結合、role 列
  (pipeline/diagnostics/tests)を付与。TCC・BiocManager・lattice・survival・Matrix を除外、
  brunnermunzel・fgsea・testthat はテストの参照実装として残す。凡例を改訂(英日)
- 2026-08-29: 表の向きを「列数」から**自然幅の見積り**へ(研究者指摘 — S6・S7 は列数が少なくても
  ID・長い見出しで横に長い): 列ごとに max(折返し不能な最長セル, 見出し(2 行許容))×1.8 mm を合算し、
  140 mm 超で横置き。結果 = 本文 Table 2 横/1・3 縦、Supp S1・S4・S6・S7 横/S2・S3・S5 縦
  (区間 PLP / PLPLPLP、全区間フッタ)。判定値はビルドログに出力
- 2026-08-29: フェーズ2 着手 — References の「(PMID n)」注記を投稿版生成時に除去(BJC の Vancouver
  書式は DOI まで。本文 31・Supp 42 件とも残存 0)。**カバーレター起草**(paper/cover_letter.md、GTA
  必須要素 = BJC 限定投稿・未発表・他誌未投稿/対応著者の所属連絡先/COI 言明/重要性と読者層。数値は
  N-16・N-20・N-41 と同一。研究者確認 = 日付・プレプリント有無・署名)
- 2026-08-29: **Detours et al. 2005・2007(Br J Cancer)を Background の先行研究文へ追加**(研究者 Go —
  カバーレターで BJC 既刊との系譜に触れる以上、本文で引かないのは不整合): 「早期のゲノム網羅的比較は
...
- 2026-08-29: カバーレターを同一ビルドの 5・6 ファイル目として docx 化(英語=編集部宛の書簡体裁:
  行番号・頁番号なし、md 冒頭の管理メモは除去/日本語参考訳 cover_letter_ja.md を新設、同期規則の
  対象に追加)。原稿 docx には混ぜない(投稿形の体裁と取り忘れ防止)。共有対は 6 ファイル同一タグ
...
- 2026-08-31: Supp の BRAF 層構成文を "consisted of BRAF V600E-positive cases" へ(研究者 Go)。
  訳同期済み。語数影響なし(Supp)
- 2026-08-31: カバーレターの ORA/較正帰無/一致の複合文を4文に分離(研究者 Go — 冒頭前置詞句の
...
- 2026-09-02: 参考訳 3 本に訳固有の必要 6 件・推奨 7 件を適用(研究者 Go): dose-zero 表記統一・「放射線特異的」の係り・
  線量付与・偏 Spearman・compared with の中立化・変換/寄与/チューニング/導かれる・parse/controlled-access/回収・
  オンコジーン。英語は不変、docx token 差 0。続けて任意 3 件(チョルノービリ事故後の甲状腺癌・高発現×低発現ペア・
  「仮説を担う」)を適用、員数・log CPM・主導的な記述例は文意が伝わるため見送り(研究者裁定)
- 2026-09-02: **語数の計数範囲を確定**(研究者計数): 本文 = Background 見出し〜Discussion 末尾・見出し込み、Word で 4,904
  (余裕 96。空白区切りは 4882 — 差は en dash 結合を Word が 2 語に数えるため)。宣言節(Additional Information)は
  約 259 語で、BJC 規定文言(abstract・references・figure legends のみ除外)上は算入可否が未確定 — 算入なら上限超過。
  投稿システム/規定/編集部で確認するまで本文の削減には着手しない。**make_docx 修正**: 宣言節で「案: 」接頭辞の行が
  CJK 行除去で丸ごと脱落していた(Data/Code availability の 1 行目 25 語)不具合を修正、再生成で復元を確認
- 2026-09-02: **BJC GTA(研究者提供の HTML、当日版)との照合で 3 件を是正**(研究者 Go): 倫理節にヘルシンキ宣言の一文を追加(人由来
  データの必須 3 点のうち欠落分)/競合利益を指定定型文 "The authors declare no conflict of interest" へ/Code availability 節を
...
- **提出パッケージ現況(2026-08-29)**: 済 = Word 本文(投稿形・タイトルページ・節順・表整形・添字・図
  ...
  最終承認・表 1〜3 の個別ファイル化(投稿時)・公開リポジトリと DOI(Data/Code availability 記入)・
  フェーズ6 再構築確認・投稿システム側の入力(キーワード・推薦査読者があれば)
